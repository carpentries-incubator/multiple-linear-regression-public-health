...
> > ```
> > 
> > Since $R^2_{adj} = 0.34$, our model accounts for approximately 34% of the variation in
...
```{r homoscedasticity example, message = FALSE}
...
> 
> ```{r testosterone challenge interact_plot, echo = FALSE}
> model <- dat %>%
>   filter(between(Age, 10, 18)) %>%
>   lm(formula = Testosterone ~ Age * Sex)
>   
> interact_plot(model, pred = Age, modx = Sex,
>             plot.points = TRUE, interval = TRUE,
>             point.size = 0.7) +
>   ylab("Testosterone") +
>   xlab("Age") 
...
> 
> ```{r diagnostic plots testosterone, echo = FALSE, message = FALSE}
> residualData <- tibble(resid = resid(model),
>                     fitted = fitted(model),
>                     age = model$model$Age,
>                     sex = model$model$Sex)
> 
...
> > ```{r updated diagnostic plot testosterone, echo = FALSE, message = FALSE}
> > p2 <- ggplot(residualData, aes(x = age, y = resid, colour = sex)) +
> >  geom_point(alpha = 0.3) +
> >  geom_smooth()
> > 
